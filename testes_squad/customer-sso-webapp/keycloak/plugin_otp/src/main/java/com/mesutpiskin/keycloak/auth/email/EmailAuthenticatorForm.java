...
public class EmailAuthenticatorForm implements Authenticator {
    ...
    private void generateAndSendEmailCode(AuthenticationFlowContext context) {
        ...
        switch(context.getRealm().getOTPPolicy().getDigits()) {
            // Aqui definimos a mascara que sera utilizado no OTP, capturamos a info vindo o realm settings dentro do admin do keycloak.
            case 6:
                mask_otp = 999999;
                break;
            case 9:
        ...
        int emailCode = ThreadLocalRandom.current().nextInt(mask_otp);
        log.debugf("Sending email code OTP: code=%s mask_used=%s", emailCode, mask_otp);
